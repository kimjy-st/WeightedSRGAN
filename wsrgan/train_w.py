...
def train( 
    ...
    while batch_data is not None:
        ...
        batch_adv_loss = []
        for i in range(16):
            gt_pil = T.ToPILImage()(gt[i])
            sr_pil = T.ToPILImage()(sr[i])
            losses = np.array([])

            srpatch, freqlist = image_crop(sr_pil, gt_pil)
            srpatchtensor = torch.stack(srpatch)
            #srpatchtensor = srpatchtensor.reshape(1,3,96,96)
            freqw = np.array([])

            for index in range(len(freqlist)):
                w = freq_weight1_1(freqlist[index])
                freqw = np.append(freqw,w)
                w = 0

                img_real_label = real_label[i].repeat(1, 1)
                srpatchtensor_i = srpatchtensor[index].to(srgan_config.device)
                srpatchtensor_i = srpatchtensor_i.reshape(1,3,32,32)
  
                patch_adv_loss = adversarial_criterion(d_model(srpatchtensor_i),img_real_label).to("cpu")
                losses = np.append(losses, patch_adv_loss)
        ...
        g_loss = pixel_loss + 0 *content_loss + adversarial_loss
        # Call the gradient scaling function in the mixed precision API to
        # back-propagate the gradient information of the fake samples
        g_loss.backward()
