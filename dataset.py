...
class LRtraindataset(Dataset):
    ...
    # Read a batch of image data
        ...
        tensor = T.ToTensor()(HR_image)
        W = tensor.size()[2]
        H = tensor.size()[1]
        LR_image_tensor = tensor.resize(3,int(round(H/self.upscale_factor)), int(round(W/self.upscale_factor)))
        LR_image = T.ToPILImage()(LR_image_tensor)
